...
void Ztt2(TString req_period = "all",int req_prongs =0,int req_ETA=0,int req_ID=2
	  ,int  b_trig=0
	  ,TString s_var="tau_0_jet_bdt_score" , int hist_Xbin=20 , double hist_Xbegin=0.6 , double hist_Xend=1
	  //,TString s_var="tau_0_pt" , int hist_Xbin=32 , double hist_Xbegin=20 , double hist_Xend=100
	  //,TString s_var="tau_0_pt" , int hist_Xbin=100 , double hist_Xbegin=25 , double hist_Xend=125
	  //,TString s_var="TMath::Abs(tau_0_truth_pdgId)" , int hist_Xbin=50 , double hist_Xbegin=-20.5 , double hist_Xend=29.5
	  ,int req_PT=0
	  )
{
...
#include "selection.h"    
  //tree_data->Scan("n_bjets:tau_0_pt:lep_0_iso_wp:lephad_vis_mass:tau_0_jet_bdt_tight:lephad_mt_lep0_met","tau_0_pt>500");
  ...
  else if(14==region)s_req=s_REQ_Z+"&&n_bjets>0";
  
  //s_req=s_req+"&&n_avg_int_cor<16";
  //s_req=s_req+"&& 16<n_avg_int_cor &&n_avg_int_cor<20";
  //s_req=s_req+"&&20<n_avg_int_cor";
  ...
  else if(14==region)selection_REGION="Zbtag";  
  ...
  float temp_rQCD[2][2]; //trig //pt
  float temp_kwOS[2][2];
  float temp_kwSS[2][2];
  
  bool trigger_factors=1;
  ...
  if(trigger_factors){
    temp_rQCD[1][0]=load_rQCD[1][0]; // if you need, change first bin
    temp_kwOS[1][0]=load_kWOS[1][0]; // if you need, change first bin
    temp_kwSS[1][0]=load_kWSS[1][0]; // if you need, change first bin
    temp_rQCD[1][1]=load_rQCD[1][1]; // if you need, change first bin
    temp_kwOS[1][1]=load_kWOS[1][1]; // if you need, change first bin
    temp_kwSS[1][1]=load_kWSS[1][1]; // if you need, change first bin
  }
  ...
  TH1D* h_361108f[2][2][2]; // trigger , OSSS ,Pt //FakeTau contribution from ZTT
  for(short ii=0; ii<1; ii++ ){
    for(short ll=0; ll<2; ll++ ){
	for(short jj=0; jj<2; jj++ ){
	  h_361108f[ii][ll][jj]= new TH1D
	    (Form("h_361108f_%d%d%d",ii,ll,jj),Form("h_361108f_%d%d%d",ii,ll,jj)
    ...
    TH1D* h_36110[1][10][2][2]; // trigger , samples , OSSS ,Pt
  ...
  //sum   
  TH1D* h_W  =new TH1D("h_W"  ,"h_W"  ,hist_Xbin,hist_Xbegin,hist_Xend);  
  TH1D* h_Zll=new TH1D("h_Zll","h_Zll",hist_Xbin,hist_Xbegin,hist_Xend);  
  TH1D* h_sig=new TH1D("h_sig","h_sig",hist_Xbin,hist_Xbegin,hist_Xend);  
  TH1D* h_tt =new TH1D("h_tt" ,"h_tt" ,hist_Xbin,hist_Xbegin,hist_Xend);  
  // sum OS
  TH1D* h_W_OS  =new TH1D("h_W_OS"  ,"h_W_OS"  ,hist_Xbin,hist_Xbegin,hist_Xend);  
  TH1D* h_Zll_OS=new TH1D("h_Zll_OS","h_Zll_OS",hist_Xbin,hist_Xbegin,hist_Xend);  
  TH1D* h_sig_OS=new TH1D("h_sig_OS","h_sig_OS",hist_Xbin,hist_Xbegin,hist_Xend);  
  TH1D* h_tt_OS =new TH1D("h_tt_OS" ,"h_tt_OS" ,hist_Xbin,hist_Xbegin,hist_Xend);  
  // sum SS  
  TH1D* h_W_SS  =new TH1D("h_W_SS"  ,"h_W_SS"  ,hist_Xbin,hist_Xbegin,hist_Xend);  
  TH1D* h_Zll_SS=new TH1D("h_Zll_SS","h_Zll_SS",hist_Xbin,hist_Xbegin,hist_Xend);  
  TH1D* h_sig_SS=new TH1D("h_sig_SS","h_sig_SS",hist_Xbin,hist_Xbegin,hist_Xend);  
  TH1D* h_tt_SS =new TH1D("h_tt_SS" ,"h_tt_SS" ,hist_Xbin,hist_Xbegin,hist_Xend);    
  // DATA
  TH1D* h_data    =new TH1D("h_data"    ,"h_data"    ,hist_Xbin,hist_Xbegin,hist_Xend);    
  TH1D* h_data_OS =new TH1D("h_data_OS" ,"h_data_OS" ,hist_Xbin,hist_Xbegin,hist_Xend);    
  TH1D* h_data_SS =new TH1D("h_data_SS" ,"h_data_SS" ,hist_Xbin,hist_Xbegin,hist_Xend);    
  TH1D* h_data_SS2=new TH1D("h_data_SS2","h_data_SS2" ,hist_Xbin,hist_Xbegin,hist_Xend);    
  ...
  TH1D* h_MC  =new TH1D("h_MC"  ,"h_MC"  ,hist_Xbin,hist_Xbegin,hist_Xend);  
  TH1D* h_MC_OS  =new TH1D("h_MC_OS"  ,"h_MC_OS"  ,hist_Xbin,hist_Xbegin,hist_Xend);  
  TH1D* h_MC_SS  =new TH1D("h_MC_SS"  ,"h_MC_SS"  ,hist_Xbin,hist_Xbegin,hist_Xend);  
  
  //TH1D* h_bkg  =new TH1D("h_bkg"  ,"h_bkg"  ,hist_Xbin,hist_Xbegin,hist_Xend);  
  TH1D* h_bkg  =new TH1D("h_bkg"  ,"h_bkg"  ,hist_Xbin,hist_Xbegin,hist_Xend);  
  ...
  TString rootfile_name="./triggerZtt_"+selection_REGION + "_" + req_period;
  ...
  CAN->cd(2);
  TH1D* h_ratio_OS2bkg  =new TH1D("h_ratio_OS2bkg","h_ratio_OS2bkg",hist_Xbin,hist_Xbegin,hist_Xend);  
  ...
  TH1D* h_ratioMC  =new TH1D("h_ratioMC","h_ratioMC",hist_Xbin,hist_Xbegin,hist_Xend);  
  hist_custom(h_ratioMC,1);
  TH1D* h_ratioDA  =new TH1D("h_ratioDA","h_ratioDA",hist_Xbin,hist_Xbegin,hist_Xend);  
  ...
  // h_ratioMC->SetMaximum(1.5+0.5);
  // h_ratioMC->SetMinimum(0.5-0.5);
