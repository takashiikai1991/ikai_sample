...
void QCD2(TString req_period = "all",int req_prongs =0,int req_ETA=0,int req_ID=2
	  ,int  b_trig=0
	  ,TString s_var="tau_0_jet_bdt_score" , int hist_Xbin=25 , double hist_Xbegin=0.6 , double hist_Xend=1
	  ,int req_PT=0
	  )
{
  int  region=3;//pre,W,qcd,Z,Z(w/omass) 1-5
  bool not_useSF=0;// before or after rQCD factor
...
#include "selection.h"    
  ...
  temp_kwOS[0][0]=1; temp_kwSS[0][0]=1;  
  temp_kwOS[0][1]=1; temp_kwSS[0][1]=1;  
  ...
  // if(not_useSF) 
  //   LABEL_NAME(7,0.92,"Before apply rQCD factors");
  // else 
  //   LABEL_NAME(7,0.92," After apply rQCD factors");
  ...
  if(not_useSF) outputpdf+="_norQCD_";
