...
void Trig__others(TString req_period = "all",int req_prongs =0,int req_ETA=0,int req_ID=2 , int b_trig=25)
{   
  ...
  // double hist_Xrange[X_BINS]={25.,28,30,32,34,36,39,43,52,64,80,100,150,300}; 
  
  // s_var="tau_0_eta"; treat_UF_bin=1;
  // const int X_BINS = 13;  int hist_Xbin = X_BINS-1;
  // double hist_Xrange[X_BINS]={-3,-2.5,-2,-1.5,-1,-0.5,0,0.5,1,1.5,2,2.5,3 }; 
  
  // s_var="tau_0_phi"; treat_UF_bin=1;
  // const int X_BINS = 15;  int hist_Xbin = X_BINS-1;
  // double hist_Xrange[X_BINS]={-3.5,-3.2,-2.5,-2,-1.5,-1,-0.5,0,0.5,1,1.5,2,2.5,3.2,3.5 }; 
  
  // s_var="tau_0_decay_mode"; 
  // const int X_BINS = 7;  int hist_Xbin = X_BINS-1;
  // double hist_Xrange[X_BINS]={-0.5,0.5,1.5,2.5,3.5,4.5,5.5 }; 
  
  // s_var="n_vx"; treat_UF_bin=1;
  // const int X_BINS = 10;  int hist_Xbin = X_BINS-1;
  // double hist_Xrange[X_BINS]={0,4,6,8,10,12,14,16,18,20 }; 
  
  s_var="n_avg_int_cor"; treat_UF_bin=1;
  // const int X_BINS = 7;  int hist_Xbin = X_BINS-1;
  // double hist_Xrange[X_BINS]={6,9,11,12,13,14,16 }; 
  const int X_BINS = 10;  int hist_Xbin = X_BINS-1;
  double hist_Xrange[X_BINS]={6,9,11,13,15,17,19,21,23,25 }; 
...
#include "selection.h"
    ...
    if(12==b_trig)
      ...
      //s_trigger="&& HLT_tau25_medium1_tracktwo==1 && tau_0_trig_HLT_tau25_medium1_tracktwo==1";
      s_trigger="&& HLT_tau25_medium1_tracktwo_resurrected==1 ";
    else if(35==b_trig)
      s_trigger="&& HLT_tau35_medium1_tracktwo==1 && tau_0_trig_HLT_tau35_medium1_tracktwo==1";
    else if(50==b_trig)
      s_trigger="&& HLT_tau50_medium1_tracktwo_L1TAU12==1 && tau_0_trig_HLT_tau50_medium1_tracktwo_L1TAU12==1";
    else if(80==b_trig)
      s_trigger="&& HLT_tau80_medium1_tracktwo_L1TAU60==1 && tau_0_trig_HLT_tau80_medium1_tracktwo_L1TAU60==1";    
    else if(125==b_trig)
      s_trigger="&& HLT_tau125_medium1_tracktwo==1 && tau_0_trig_HLT_tau125_medium1_tracktwo==1";    
    ...
    if(print_info){ 
      ...
      f_h_ev[2]=ht_W       ->IntegralAndError(1, NN_XBIN ,f_h_er[2]);
      f_h_ev[3]=ht_Zll     ->IntegralAndError(1, NN_XBIN ,f_h_er[3]);
      f_h_ev[4]=ht_tt      ->IntegralAndError(1, NN_XBIN ,f_h_er[4]);
      f_h_ev[5]=ht_data_SS ->IntegralAndError(1, NN_XBIN ,f_h_er[5]);
      f_h_ev[6]=ht_sig     ->IntegralAndError(1, NN_XBIN ,f_h_er[6]);
    ...
    leg->SetTextFont(42);
    leg->SetBorderSize(0);
    leg->AddEntry(h_sig, "Z#rightarrow#tau#tau", "F");//LPFE
    leg->AddEntry(h_data_OS, "Data 2015", "LEP");
    leg->Draw();
    TString latex_draw="";
