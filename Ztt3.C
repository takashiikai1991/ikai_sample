...
void Ztt3(TString req_period = "all",int req_prongs =0,int req_ETA=0,int req_ID=2
	  ,int  b_trig=0
...
#include "selection.h"    
  ...
  if(25==b_trig)
    ...
    s_req="(" +s_req +"&& HLT_tau35_medium1_tracktwo==1 && tau_0_trig_HLT_tau35_medium1_tracktwo==1"+")";
  
  TString _OS="&& lephad_qxq==-1";
  TString _SS="&& lephad_qxq==+1";
  TString s_Pt[3] ={""};//"&&tau_0_pt<35";  // smaller 
  s_Pt[0] ="&& 25<tau_0_pt && tau_0_pt<30";
  s_Pt[1] ="&& 30<tau_0_pt && tau_0_pt<40";
  s_Pt[2] ="&& 40<tau_0_pt ";
  
  float temp_rQCD[2][3]={1}; //trig //pt
  float temp_kwOS[2][3]={1};
  float temp_kwSS[2][3]={1};
  ...
  temp_rQCD[0][0] = set_rQCD("mean",req_period,req_prongs,req_ETA,req_ID ,3,0);   
  temp_rQCD[0][1] = set_rQCD("mean",req_period,req_prongs,req_ETA,req_ID ,4,0);   
  temp_rQCD[0][2] = set_rQCD("mean",req_period,req_prongs,req_ETA,req_ID ,5,0);   
  temp_kwOS[0][0] = set_kW("mean",req_period,req_prongs,req_ETA,req_ID ,"OS",3,0);   
  temp_kwOS[0][1] = set_kW("mean",req_period,req_prongs,req_ETA,req_ID ,"OS",4,0);   
  temp_kwOS[0][2] = set_kW("mean",req_period,req_prongs,req_ETA,req_ID ,"OS",5,0);   
  temp_kwSS[0][0] = set_kW("mean",req_period,req_prongs,req_ETA,req_ID ,"SS",3,0);   
  temp_kwSS[0][1] = set_kW("mean",req_period,req_prongs,req_ETA,req_ID ,"SS",4,0);   
  temp_kwSS[0][2] = set_kW("mean",req_period,req_prongs,req_ETA,req_ID ,"SS",5,0);   

  temp_rQCD[1][0] = set_rQCD("mean",req_period,req_prongs,req_ETA,req_ID ,3,0);   
  temp_rQCD[1][1] = set_rQCD("mean",req_period,req_prongs,req_ETA,req_ID ,4,0);   
  temp_rQCD[1][2] = set_rQCD("mean",req_period,req_prongs,req_ETA,req_ID ,5,0);   
  temp_kwOS[1][0] = set_kW("mean",req_period,req_prongs,req_ETA,req_ID ,"OS",3,0);   
  temp_kwOS[1][1] = set_kW("mean",req_period,req_prongs,req_ETA,req_ID ,"OS",4,0);   
  temp_kwOS[1][2] = set_kW("mean",req_period,req_prongs,req_ETA,req_ID ,"OS",5,0);   
  temp_kwSS[1][0] = set_kW("mean",req_period,req_prongs,req_ETA,req_ID ,"SS",3,0);   
  temp_kwSS[1][1] = set_kW("mean",req_period,req_prongs,req_ETA,req_ID ,"SS",4,0);   
  temp_kwSS[1][2] = set_kW("mean",req_period,req_prongs,req_ETA,req_ID ,"SS",5,0);   
  ...
  printf("FACTOR (pt1) = %1.3f @ %1.3f @ %1.3f \n",  temp_rQCD[0][0] , temp_kwOS[0][0], temp_kwSS[0][0]  );
  printf("FACTOR (pt2) = %1.3f @ %1.3f @ %1.3f \n",  temp_rQCD[0][1] , temp_kwOS[0][1], temp_kwSS[0][1]  );
  printf("FACTOR (pt3) = %1.3f @ %1.3f @ %1.3f \n",  temp_rQCD[0][2] , temp_kwOS[0][2], temp_kwSS[0][2]  );  

  TH1F* h_361108f[2][2][3]; // trigger , OSSS ,Pt //FakeTau contribution from ZTT
  for(short ii=0; ii<1; ii++ ){
    for(short ll=0; ll<2; ll++ ){
      for(short jj=0; jj<3; jj++ ){
	h_361108f[ii][ll][jj]= new TH1F
	...
	if(0==jj)      temp_req+=s_Pt[0];      //low  pt
	else if(1==jj) temp_req+=s_Pt[1];     //high pt
	else if(2==jj) temp_req+=s_Pt[2];     //high pt
    ...
    TH1F* h_36110[1][10][2][3]; // trigger , samples , OSSS ,Pt
  ...
  TH1F* h_data_SS3=new TH1F("h_data_SS3","h_data_SS3" ,hist_Xbin,hist_Xbegin,hist_Xend);    
  set_data(h_data_OS , tree_data  ,s_req+_OS         , s_var);	  
  set_data(h_data_SS , tree_data  ,s_req+_SS+s_Pt[0] , s_var);	  
  set_data(h_data_SS2, tree_data  ,s_req+_SS+s_Pt[1] , s_var);	  
  set_data(h_data_SS3, tree_data  ,s_req+_SS+s_Pt[2] , s_var);	  
  ...
  for(short kk=0; kk<11; kk++ ){
	...
	if(0==ll){//OS
	  ...
	  else if(8==kk)     h_sig_SS -> Add(h_36110[ii][kk][ll][jj],temp_rQCD[ii][jj]);
	  else if(9==kk)     h_tt_SS  -> Add(h_36110[ii][kk][ll][jj],temp_rQCD[ii][jj]);
	  else if(10==kk)    h_Zll_SS -> Add(  h_361108f[ii][ll][jj],temp_rQCD[ii][jj]);
	}
	printf("%d%,%d%,%d\n",kk,ll,jj);
      }}}
  //W +jets
  ...
  h_data_SS3->Scale(temp_rQCD[0][2]);
  h_data_SS->Add(h_data_SS2);
  h_data_SS->Add(h_data_SS3);
  //////////////////////////////////fasdasdfsdfafae
  ...
  h_ratioMC->SetMaximum(1.25);
  h_ratioMC->SetMinimum(0.75);
  ...
  TString outputpdf=
    selection_REGION + "3" + req_period +
    //Form("_%1.1dprong_%1.1dETA_%1.1dBDT",req_prongs,req_ETA,req_ID)  +"__"+OUTPUT;
    Form("_%1.1dprong_%1.1dETA_%1.1dBDT_%1.1dtaupt",req_prongs,req_ETA,req_ID,req_PT)  +"__"+OUTPUT;
  if(tree_NAME_sys!="NOMINAL")outputpdf+=tree_NAME_sys;
  
  if(0==b_trig)CAN->SaveAs("./pdf/" +outputpdf+".pdf");
  else CAN->SaveAs("./Npdf/"+outputpdf+"_trig25.pdf");
