...
void Wjets2(TString req_period = "all",int req_prongs =0,int req_ETA=0,int req_ID=2
	    ,int  b_trig=0
	    ,int OSSS=2
...
{   
  ...
  bool not_useSF=0;// before or after kw factor
...
#include "selection.h"    
  ...
  if(2==OSSS)     selection_REGION+="OS";  
  else if(3==OSSS)selection_REGION+="SS";  
  ...
  if(not_useSF){
    temp_rQCD[0][0]=1; temp_kwOS[0][0]=1; temp_kwSS[0][0]=1;  
    temp_rQCD[0][1]=1; temp_kwOS[0][1]=1; temp_kwSS[0][1]=1;  
    printf("==================\n");
    printf("==================\n");
    printf(" Not All Use SFs \n");
    printf("==================\n");
    printf("==================\n");
  }
  ...
  TH1F* h_361108f[2][2][2]; // trigger , OSSS ,Pt //FakeTau contribution from ZTT
  for(short ii=0; ii<1; ii++ ){
    ...
    TH1F* h_36110[1][10][2][2]; // trigger , samples , OSSS ,Pt
  ...
  if(1==OSSS){
    ...
    h_stack->Add(h_W_OS,"hist");
    h_stack->Add(h_Zll_OS,"hist");
    h_stack->Add(h_tt_OS,"hist");
    h_stack->Add(h_sig_OS,"hist");
  } else if(3==OSSS){
    h_stack->Add(h_W_SS,"hist");
    h_stack->Add(h_Zll_SS,"hist");
    h_stack->Add(h_tt_SS,"hist");
    h_stack->Add(h_sig_SS,"hist");
  }
  if(1>0){
  ...
  if(2==OSSS)     leg_option=" (OS)";
  else if(3==OSSS)leg_option=" (SS)";
  ...
  if(not_useSF) 
    LABEL_NAME(7,0.92,"Before apply kw factors");
  else 
    LABEL_NAME(7,0.92," After apply kw factors");
