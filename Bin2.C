...
void Bin2(TString req_period = "all",int req_prongs =0,int req_ETA=0,int req_ID=2
	  ,int  b_trig=0
	  ...
	  )
{
  ...
  // ===================================================================================================
  const int X_BINS = 14-7; 
  hist_Xbin = X_BINS-1;
  double hist_Xrange[X_BINS]={43,52,64,80,100,150,300}; //pt binning
...
#include "selection.h"    
  ...
  h_bkg->Draw("E2sames text20");//MC error band
  h_data_OS->SetBarOffset(1.5);
  h_data_OS->Draw("sames text50");//re-draw
  // int NN_XBIN=h_data_OS->GetNbinsX();
  // TText *Ttex=new TText( 1,1,"" );
  // Ttex->SetTextSize (0.03);
  // for(short kk=1; kk<NN_XBIN+1; kk++ ){
  //   double ii_x=h_data_OS->GetBinCenter(kk);
  //   double ii_y=h_data_OS->GetBinContent(kk);
  //   double ii_e=h_data_OS->GetBinError(kk);
  //   TString s_ii_e=Form("(+- %3.1f)",ii_e);
  //   Ttex->DrawText( ii_x,ii_y/1.5, s_ii_e );
  // }
  //Ttex->DrawText( 100,10, "a" );
